...
| Verified against | **media-mcp 0.7.15** (`vagaa-media` 0.3.20: director_accept seam gate + storyboard_direct(review=); storyboard_status/fetch read director runs; per-shot ref2v enabled; director packs round-trip with reference videos; plan character references by asset id; `storyboard_direct` plan→one director job; director console `director_run`, characters table, `refine=latent_upscale`), **Nólë v1.10.2+dgx.20260828.24** (`vagaa-web-evidence`) |
