...
verified_against: media-mcp 0.7.56 (2026-09-19)
...
- **Truly continuous shots (one action, one scene) must render their final at native size (`width=1344, height=768`, no `refine`)**: both refine modes run per segment after continuity was established at draft size, so every real seam becomes a one-frame jump of ~7× the median frame difference and the audio at a join can clip to 0 dB (C03 finals A/B, `reference/c03-short-20260915/README.md`). Cut-style storyboards are not seams: see "Which final to make".
...
- Measured limits (each number has a file behind it; `tools/evals/hypotheses-20260909/RESULTS.md` = R):
  ...
  · Finals (R E3/E6/E7, sharpness : time vs 480p+latent_upscale = 1 : 1): `refine="upscale"` (author's second pass) 1.6× : 1.8–3.5×; native 768p turbo 1.6× : 2.5–3×; official 20-step native 2.6–5× : 6–7×. **Which final to make** (the user just says 出正式版; pick by whether the shots are truly continuous): shots that continue ONE action in ONE scene (fl2v/t2v seams, C03 2026-09-16) → native `width=1344, height=768`, no `refine` — both refine modes re-sample per segment and turn every real seam into a one-frame jump; shots that are separate framings / cuts (a character-table r2v storyboard, C04 2026-09-16: joins unchanged, refine ≈2× sharper than native at equal ~40 min) → `refine="upscale"` (pass `continuity=False`: with the pin left on, a native re-render blended the cut at 5 s into a visible ghost — user-observed on C04 native as-9ab4f0970fa6, while the refine render of the same seed kept a clean cut); one segment → `refine="latent_upscale"` (fastest, frame-for-frame the draft; C02/C08); the user asks for sharper (更清晰/二采) on a single segment → `refine="upscale"`; showcase (展示级/官方 20 步) → 20-step native. To force a path the user says 原生尺寸出 / 放大出 / 二采出. Fast tier for any finished draft: `video_upscale(source)` (SPAN 2x, ~2 min, joins untouched, ≈2x draft sharpness). Then level the audio: `video_audio_level(source=<run_id or asset>)` reads the run's own segments, evens the per-shot loudness and dissolves every cut through a 0 dB ambience bed with 150 ms edge fades (`reference/audio-level-20260916/`, user-approved 2026-09-17). Evidence: `reference/c03-short-20260915/README.md`, `reference/qwen-trial-20260914/cc-G/NOTES.md`.
