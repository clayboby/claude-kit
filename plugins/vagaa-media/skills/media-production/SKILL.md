...
2. **Review.** `video_review(source=job_id)` / `image_review(source=job_id)` → five 1–5 scores + `overall` + one suggestion from the Qwen vision
   ...
   *Levels (0.7.50):* a multi-shot film comes back with 10–16 dB steps at the cuts, ~50 ms holes at every join and the odd shot clipping — H3 renders each shot's audio alone; level wording changed nothing in two same-seed re-renders (C04 v3, S9 v2; n=2). Finish with `video_audio_level(source=<run or asset>)`: one measured gain per shot (capped by `max_gain_db`, true-peak headroom kept), a looped ambience bed from one shot's interior at `bed_db` (default 0 = as loud as the shots) and `edge_fade_s` (default 0.15 s) fade-out/fade-in around every cut so the join passes through the bed — S9 v3 cut dips 9/5/10/5/9 → 2/2/3/1/2 dB, user-approved 2026-09-17; check `max_volume` and `cut_holes`. Texture differences between shots come from what the picture shows (a bus arriving), not from soundscape wording (S9 v4 2026-09-17: identical soundscape paragraphs, same texture).
