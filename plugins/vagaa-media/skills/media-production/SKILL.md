...
Images — pick the ROUTE first, first match wins (`presets_list(brief=true).image_guide`): (1) a real-looking PHOTO that must carry exact words (shop front, product poster, menu board, ad) = TWO calls: `image_submit(prompt, preset="krea-default", aspect_ratio="2:3", seed)` for the photo with NO text in it, then `image_edit(images=[<that job_id>], instruction='add the title "…" and the date "…" in the empty sky')` (Krea2 alone misspells Chinese; `qi21-text` alone looks less like a photo); (2) typography IS the picture (graphic poster, flyer, infographic, UI, menu design) → `preset="qi21-text"`, quote the words; (3) combine / keep / change EXISTING pictures → `image_edit`; (4) no exact text, photoreal / aesthetic / fast → `krea-default` (`krea-169` = 1344×768, 7:4; exact 16:9 = `width=1024, height=576`). **Posters and covers are portrait 2:3** (`aspect_ratio="2:3"`; Krea: 832×1248, or `width=1024, height=1536` for a larger print). Also `"qi21-scene"` (crowds, cities), `"qi21-rgba"` (transparent PNG), `"qi21-t2i"` (long structured prompt), `"qi21-quality"` (one hard final). `qi21-text` rewrites with the official Qwen prompt enhancer first (+15–45 s; `rewrite="off"` skips it); poll with `job_wait`, then `image_fetch(job_id)`.
