...
2. **Review.** `video_review(source=job_id)` / `image_review(source=job_id)` → five 1–5 scores + `overall` + one suggestion from the Qwen vision
   ...
   *Levels (0.7.49):* a multi-shot film comes back with 10–16 dB steps at the cuts and the odd shot clipping — H3 renders each shot's audio alone; level wording ("no louder than the other shots", "the same constant level") changed nothing in two same-seed re-renders (C04 v3, S9 v2; n=2, provisional). Finish with `video_audio_level(source=<run or asset>)`: one measured gain per shot (capped by its true peak) plus a looped ambience bed from shot 1 that fills the ~50 ms hole the Director leaves at every cut, picture untouched, seconds. Check `max_volume` and the returned `cut_holes` before delivery.
