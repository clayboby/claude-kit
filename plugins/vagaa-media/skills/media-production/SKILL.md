...
2. **Review.** `video_review(source=job_id)` / `image_review(source=job_id)` → five 1–5 scores + `overall` + one suggestion from the Qwen vision
   ...
   *Levels (0.7.51):* a multi-shot film comes back with 10–16 dB steps at the cuts, ~50 ms holes at every join and the odd shot clipping — H3 renders each shot's audio alone; level wording changed nothing in two same-seed re-renders (C04 v3, S9 v2; n=2). Finish with `video_audio_level(source=<run or asset>)`: one measured gain per shot (true-peak headroom kept), `edge_fade_s` (default 0.15 s) fade-out/fade-in around every cut, and a bed under the whole film — by default the LONGEST stationary stretch of shot 1 looped with a crossfade seam (0.7.50 looped the whole shot interior and repeated a knock every 4.7 s on C04: "全程有个背景的重复的声音"); better, pass `ambience=<a 30–60 s room tone from music_submit preset sfx>` at least as long as the film (the studio practice; never looped) and `music=<one music_submit track>` at `music_db` (default −14) instead of per-segment `non_diegetic_music`. The result reports `cut_holes`, `bed_periodicity` (should stay near the raw film's) and `mix_true_peak_dbtp` (≤ −2). Texture differences between shots come from what the picture shows, not from soundscape wording (S9 v4 2026-09-17).
