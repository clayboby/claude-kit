...
| a causal event never happens across cut-style segments (the bag is knocked in one shot, the next shot pours flour by itself) | each `continuity=false` segment samples its own action; wording did not fix it in two versions (held-out Flour Crown 2026-09-17, director and storyboard) | stage the event as a keyframe chain: first frame = state before, last frame = `image_edit` of the SAME image changing only the result, the event in one `fl2v` segment (n=1: the chain "lift → rim knocks bag → bag tips → flour" then played in order; the segment after it started from the result frame). Sound events (a hum that stops) are cut in post, not prompted |
