...
| a new image in a chosen composition / size | yes | `image_submit(prompt, preset="krea-default"|"krea-169", seed, width, height)` for photoreal / aesthetic; `preset="qi21-text"` (exact words), `"qi21-scene"` (crowds, cities), `"qi21-rgba"` (transparent PNG) — `media-production` owns the choice; → `image_review` |
...
| best-looking poster / ad with exact text | yes, two steps | `image_submit(preset="krea-default"|"krea-169")` for the photo → `image_edit(images=[<that job or asset>], instruction='add the title "…" and the date "…" in the empty space at the top')`. One-step alternative: `image_submit(preset="qi21-text")` (Qwen draws both; its photo realism is weaker than Krea2's) |
